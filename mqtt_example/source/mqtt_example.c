...
#define EXAMPLE_MQTT_USER "jmaya371"
#define EXAMPLE_MQTT_PSWD "aio_wlFw30KCk8z2o3BkXiE8gpD4oNQd"
...
static void GetAutomaticHum(void);
...
uint32_t humidity_sensor = 45;
int32_t Temp_sensor = 20;
int32_t TempDes_sensor = 30;
uint32_t HumDes_sensor = 30;
uint32_t AirConditioner = 0;
uint32_t samples_cnt = 0;
bool sprinklers_on;
static uint8_t MQTT_FLAGHumedity = 0;
static uint8_t MQTT_LenDataTopic = 0;
/*******************************************************************************
 * Code
 ******************************************************************************/
...
static void mqtt_incoming_publish_cb(void *arg, const char *topic, u32_t tot_len)
{
    ...
    MQTT_LenDataTopic = tot_len;
	if (!memcmp(topic, "jmaya371/feeds/humdes", 21))
	{
		MQTT_FLAGHumedity = true;
	}
	else
	{
		MQTT_FLAGHumedity = false;
	}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    else if (MQTT_FLAGHumedity  == false){
    	uint8_t u8unit = 1;
    	TempDes_sensor = 0;
    	for (int i = (MQTT_LenDataTopic - 1); i >= 0; i--)
    	{
    		TempDes_sensor += ((int)(data[i] - '0') * u8unit);
    		u8unit = u8unit*10;
    	}
        PRINTF("\r Temperature Des: %d\r\n", TempDes_sensor);
    }
    else if (MQTT_FLAGHumedity == true)
    {
    	uint8_t u8unit = 1;
    	HumDes_sensor = 0;
    	for (int i = (MQTT_LenDataTopic - 1); i >= 0; i--)
    	{
    		HumDes_sensor += ((int)(data[i] - '0') * u8unit);
    		u8unit = u8unit*10;
    	}
        PRINTF("\r Humedad Des: %d\r\n", HumDes_sensor);
    }
...
{
    static const char *topics[] = {"jmaya371/feeds/conditioner", "jmaya371/feeds/tempdes", "jmaya371/feeds/humdes"};
    int qos[]                  	= {0,0,0};
    err_t err;
    int i;
...
static void publish_humidity(void *ctx)
{
    static const char *topic   = "jmaya371/feeds/humedity";
    static char message[10];

    LWIP_UNUSED_ARG(ctx);

    memset(message, 15, 10);
...
{
    static const char *topic   = "jmaya371/feeds/temperature";
    static char message[10];
 ...
 * @brief Publishes a message. To be called on tcpip_thread.
 */
static void publish_tempgrap(void *ctx)
{
    static const char *topic   = "jmaya371/feeds/tempgrap";
    static char message[10];
 ...
 * @brief Publishes a message. To be called on tcpip_thread.
 */
static void publish_humgrap(void *ctx)
{
    static const char *topic   = "jmaya371/feeds/humgrap";
    static char message[10];
...
static void app_thread(void *arg)
{
    ...
    while(1) {
		...
		if(uxBits & MQTT_CONNECTED_EVT ) {
			...
			{
				Temp_sensor = get_simulated_sensor(Temp_sensor, 7, -40, 50, sprinklers_on);
				humidity_sensor = get_simulated_sensor(humidity_sensor, 2, 5, 100, sprinklers_on);
			}
			else
			{
				GetAutomaticTemp();
			...
			if((samples_cnt++%3) == 2){
				...
				err |= tcpip_callback(publish_tempgrap, NULL);
				err |= tcpip_callback(publish_humgrap, NULL);
				if (err != ERR_OK)
				{
					PRINTF("Failed to invoke some publish on the tcpip_thread: %d.\r\n", err);
				}
			}
...
void GetAutomaticTemp(void)
{
	if (TempDes_sensor > Temp_sensor)
	{
		Temp_sensor += 2;
	}
	else
...
void GetAutomaticHum(void)
{
	if (HumDes_sensor > humidity_sensor)
	{
		humidity_sensor += 3;
	}
	else
